...
public class EdmondsMatching {
     ...
     */
    public static UndirectedGraph<Vertex, Edge> maximumMatching(UndirectedGraph<Vertex, Edge> g) {        
        /* Construct a new graph to hold the matching.  Fill it with the nodes
         * from the original graph, but not the edges.
         */        
        UndirectedGraph<Vertex, Edge> result = new UndirectedSparseMultigraph();
        ...
        while (true) {
             ...
             */
            List<Vertex> path = findAlternatingPath(g, result);
            if (path == null) {                
                return result;
            }
     ...
     */
    private static void updateMatching(List<Vertex> path, UndirectedGraph<Vertex, Edge> m) {
        ...
        for (int i = 0; i < path.size() - 1; ++i) {
            ...
            Edge e = m.findEdge(path.get(i), path.get(i + 1));
            if( e != null) {                
                m.removeEdge(e);                
            } else {
                Edge edge = new Edge(path.get(i), path.get(i + 1));
                m.addEdge(edge, path.get(i), path.get(i + 1), EdgeType.UNDIRECTED);                
            }
        }
    }
    ...
    private static final class NodeInformation<T> {
         ...
         */
        public NodeInformation(Vertex parent, Vertex treeRoot, boolean isOuter) {
            this.parent = parent;
    ...
    private static final class Blossom {
        ...
        public final Vertex root; // The root of the blossom; also the representative
        public final List<Vertex> cycle; // The nodes, listed in order around the cycle
        public final Set<Vertex> nodes; // The nodes, stored in a set for efficient lookup
         ...
         */
        public Blossom(Vertex root, List<Vertex> cycle, Set<Vertex> nodes) {
            this.root = root;
            this.cycle = cycle;
            this.nodes = nodes;
        }
    ...
    /**
     * Given a graph and a matching in that graph, returns an augmenting path in
     * the graph if one exists.
     *
     ...
     */
    private static List<Vertex> findAlternatingPath(UndirectedGraph<Vertex, Edge> g,
            UndirectedGraph<Vertex, Edge> m) {
         ...
         */
        Map<Vertex, NodeInformation<Vertex>> forest = new HashMap<Vertex, NodeInformation<Vertex>>();
        ...
        for (Vertex v : g.getVertices()) {
             ...
             * its own tree.
             */
            forest.put(v, new NodeInformation<Vertex>(null, v, true));

            /* Add to the worklist all edges leaving this node. */
            for(Vertex vAdj : g.getSuccessors(v)) {                
                worklist.add(new Edge(v, vAdj));
            }
        }
        ...
        while (!worklist.isEmpty()) {
            ...
            if (m.findEdge(curr.getNode1(), curr.getNode2()) != null) {
                continue;
            }

            /* Look up the information associated with the endpoints. */
            NodeInformation<Vertex> startInfo = forest.get(curr.getNode1());
            NodeInformation<Vertex> endInfo = forest.get(curr.getNode2());
            ...
            if (endInfo != null) {
                ...
                if (endInfo.isOuter && startInfo.treeRoot == endInfo.treeRoot) {
                     ...
                     */
                    List<Vertex> path = findAlternatingPath(contractGraph(g, blossom),
                ...
                } /* Case 2: Return the augmenting path from root to root. */ else if (endInfo.isOuter && startInfo.treeRoot != endInfo.treeRoot) {
                     ...
                     * it afterwards.
                     */
                    for (Vertex v = curr.getNode1(); v != null; v = forest.get(v).parent) {
                        result.add(v);
                    }
                    ...
                    for (Vertex v = curr.getNode2(); v != null; v = forest.get(v).parent) {
                        result.add(v);
                    }
             ...
             */ else {
                 ...
                 * root as its parent.
                 */
                forest.put(curr.getNode2(), new NodeInformation<Vertex>(curr.getNode1(),
                        startInfo.treeRoot,
                        false));
                 ...
                 * tree.
                 */
                Vertex v2 = curr.getNode2();
                Vertex endpoint = null;
                for(Vertex it : m.getSuccessors(v2)) {
                    endpoint = it;
                }                
                forest.put(endpoint, new NodeInformation<Vertex>(curr.getNode2(),
                        startInfo.treeRoot,
                        true));
                 ...
                 * list.
                 */
                for (Vertex fringeNode : g.getSuccessors(endpoint)) {                    
     ...
     */
    private static Blossom findBlossom(Map<Vertex, NodeInformation<Vertex>> forest,
            Edge edge) {
         ...
         * to both paths.
         */
        LinkedList<Vertex> onePath = new LinkedList<Vertex>(), twoPath = new LinkedList<Vertex>();
        for (Vertex node = edge.getNode1(); node != null; node = forest.get(node).parent) {
            onePath.addFirst(node);
        }
        for (Vertex node = edge.getNode2(); node != null; node = forest.get(node).parent) {
        ...
        return new Blossom(onePath.get(mismatch - 1), cycle, new HashSet<Vertex>(cycle));
    }
     ...
     */
    private static  UndirectedGraph<Vertex, Edge> contractGraph(UndirectedGraph<Vertex, Edge> g,
            Blossom blossom) {
        ...
        for (Vertex node : g.getVertices()) {
            ...
            for (Vertex endpoint : g.getSuccessors(node)) {
                 ...
                 */                
                Edge e = new Edge(node, endpoint);
                result.addEdge(e, node, endpoint);
            }
        }
     ...
     */
    private static List<Vertex> expandPath(List<Vertex> path,
            UndirectedGraph<Vertex, Edge> g,
            Map<Vertex, NodeInformation<Vertex>> forest,
            Blossom blossom) {
        ...
        for (int i = 0; i < path.size(); ++i) {
             ...
             */ else {
                 ...
                 * in the path.
                 */
                Vertex outNode = findNodeLeavingCycle(g, blossom, path.get(i + 1));
     ...
     */
    private static Vertex findNodeLeavingCycle(UndirectedGraph<Vertex, Edge> g,
            Blossom blossom, Vertex node) {
        /* Check each node in the blossom for a matching edge. */
        for (Vertex cycleNode : blossom.nodes) {
            Edge e = g.findEdge(cycleNode, node);
            if (e != null) {
                return cycleNode;
            }
        }
